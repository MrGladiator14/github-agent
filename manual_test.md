...
Expected: Gemini uses analyze_file_changes() tool

User: "Show me available PR templates"
Expected: Gemini lists all templates

User: "Suggest a template for a bug fix"
Expected: Gemini recommends bug.md template
```
...
Expected: Gemini shows all 3 events with details

User: "Show me the workflow status"
Expected: Gemini shows:
- CI Tests: success
...
Expected: Gemini provides formatted analysis showing:
...
Expected: Gemini creates Slack-style message about the deployment failure
```

**Prompt 3:**
```
User: "generate pr status report"
Expected: Gemini combines file changes analysis with CI/CD status
```

**Prompt 4:**
```
User: "troubleshoot workflow failure"
Expected: Gemini provides structured troubleshooting for Deploy to Production failure
```
...
4. Verify real events appear in Gemini
...
### Gemini not finding server
- Restart Gemini Code after config changes
